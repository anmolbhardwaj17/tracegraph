...
function AuthForm() {
  ...
  if (user) { router.push(redirect); return null; }
  ...
  async function handleGoogleSuccess(credentialResponse: any) {
    ...
    try {
      ...
      window.location.href = redirect; // hard redirect to pick up new auth state
    } catch (err: any) {
  ...
  const showGoogleBtn = !!GOOGLE_CLIENT_ID;
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        {showGoogleBtn ? (
          ...
          <button
            type="button"
            onClick={() => setError('Add NEXT_PUBLIC_GOOGLE_CLIENT_ID to .env to enable Google login')}
            className="w-full py-3 px-4 border border-white/10 rounded-sm text-sm text-ink-400 flex items-center justify-center gap-3 mb-4 hover:border-white/20 transition-colors"
          >
            <GoogleIcon />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder={mode === 'signup' ? 'Min 6 characters' : 'Your password'} required minLength={mode === 'signup' ? 6 : undefined}
              className="w-full px-4 py-3 bg-ink-850 border border-white/10 rounded-sm text-sm text-ink-50 placeholder:text-ink-600 focus:border-white/20 focus:outline-none" />
          </div>

          {error && (
            <div className="text-signal-critical text-xs bg-signal-critical/10 border border-signal-critical/20 rounded-sm px-3 py-2">{error}</div>
          )}
        ...
        {mode === 'signup' && (
          <div className="mt-8 border border-white/5 rounded-sm p-4">
            ...
            <ul className="space-y-2 text-xs text-ink-400">
              ...
              <li className="flex gap-2"><span className="text-signal-clean">✓</span> PDF report + IC memo export</li>
...
export default function AuthPage() {
  return (
    <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID || 'placeholder'}>
      <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-6 h-6 border-2 border-ink-700 border-t-ink-50 rounded-full animate-spin" /></div>}>
        <AuthForm />
      </Suspense>
    </GoogleOAuthProvider>
  );
